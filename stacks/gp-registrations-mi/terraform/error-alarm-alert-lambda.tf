...
resource "aws_lambda_function" "error_alarm_alert_lambda" {
  ...
  environment {
    variables = {
      ...
      CLOUDWATCH_ALARM_URL                      = "${data.aws_region.current.region}.console.aws.amazon.com/cloudwatch/home#alarmsV2:?~(alarmStateFilter~'ALARM)",
      CLOUDWATCH_DASHBOARD_URL                  = "${data.aws_region.current.region}.console.aws.amazon.com/cloudwatch/home#dashboards:name=${aws_cloudwatch_dashboard.mi_api.dashboard_name}",
    }
  }
}
